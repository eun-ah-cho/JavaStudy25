...
public class LogicalAndExam {
	...
	public static void main(String[] args) {
		...
		System.out.println("정수를 입력 하시면 대소문자나 숫자를 판단해 드릴게요!!!");
		System.out.println("유니코드표를 참고하여 프로그램을 제작하였습니다.");
		System.out.print("숫자 입력 >>> ");
		int charCode = input.nextInt(); // 키보드로 정수값이 입력되고 변수에 들어감. 
		
		// if(비교판단문){ 비교 판단문이 true 값일때 수행되는 문 } 
		// else { 비교 판단문이 false값 일때 수행되는 문 } 
		if((charCode >= 65) & (charCode <=90 )) { 
			// 입력된 숫자 값이 65 이상이고 90 이하이면 대문자 
			System.out.println("현재 입력된 숫자는 대문자 입니다. : " + (char)charCode );
		}
		
		
		else if( (charCode >=97)  && (charCode <=122 ) ) { 
			//위에서 실행된 if값이 false일 경우  또 비교를 함. 
			...
			System.out.println("현재 입력된 숫자는 유니코드이 문자형 숫자 입니다. : " + (char)charCode	);
